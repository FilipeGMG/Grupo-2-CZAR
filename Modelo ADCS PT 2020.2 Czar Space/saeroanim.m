...
%
   ...
   set(h_anim,'name','Cubesat Atitude', ...
           'renderer','z','resize','on', ...
           'position',[80 10 546 449],'clipping','off', ...
           'Tag','6DOF anim');
...
%
   handle.axes(1)=axes;
   axis(Config.axes);
   set(handle.axes(1),'visible','on','box','on', ...
           ...
           'drawmode','fast', ...
           'clipping','off');
       handle.axes(1).XLabel.String = 'Eixo X';
       handle.axes(1).YLabel.String = 'Eixo Y';
       handle.axes(1).ZLabel.String = 'Eixo Z';
...
%
   [zm,xm,ym] = miss_shape;
   handle.craft = patch(xm,ym,zm,[0 0.5 1]);
	vert = Config.craft*get(handle.craft,'vertices');
   set(handle.axes(1),'userdata',vert)
   set(handle.craft,'erasemode','nor','edgecolor',[0.1 0.1 1],'clipping','off');
%
...
% 
% 	len 		= 4;				% Total Length (no units)
% 	radius		= 4;				    % Radius of body
%  	s_fore	 	= 5;					% Start of main body (w.r.t. nose)
%    	thr_len 	= 1;				% Length of Motor exit
%  	%rad_throt	= 1.3;				% radius of motor exit
%    	l_fore=len-s_fore-thr_len;			% Length of main body
%    	c_g 			= 4;				% Position of c.g. w.r.t nose
%    
% %
% % Fore Body Shape
% %
...
% %
%   count=count+4;
% 	xcraft{count}=[0 0 0 0];
% 	ycraft{count}=[-zc_range(1) -zc_range(1)+9 -zc_range(1)+9 -zc_range(1)];
% 	zcraft{count}=[3 3 5 5]-c_g;
